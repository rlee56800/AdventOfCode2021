...
with open('Day01_Input.txt') as f:
    prev = int(next(f)) 
    for i in range(amtInputs - 1): # SORRY NOT SCALABLE
        #print(i)
        #print(amtIncreasing)
        current = int(next(f))
        if prev < current:
...
B (200, 208, 210); its sum is 618. The sum of measurements in the second window is larger than the sum
...
amtIncreasing2 = -1 # first sum adds to count, so make this -1

with open('Day01_Input.txt') as f:
    one = int(next(f)) 
    two = int(next(f))
    prevSum = 0
    for i in range(amtInputs - 2): # SORRY NOT SCALABLE
        three = int(next(f))
        sum = one + two + three
        if sum > prevSum:
            amtIncreasing2 += 1
